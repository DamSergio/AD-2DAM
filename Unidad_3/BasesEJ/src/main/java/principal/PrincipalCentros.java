...
//		consultarCentro((short) 1000);
		System.out.println("--------------------");
		actualizarCentro((short) 1050, "nuevo 10", (short) 1001, "Nueva direc");
		actualizarCentro((short) 1051, "nuevo 10", (short) 1001, "Nueva direc");
		actualizarCentro((short) 1051, "nuevo 10", (short) 10011, "Nueva direc");
		System.out.println("--------------------");
		
		sesion.close();
	}

	private static void actualizarCentro(short cen, String nom, short dir, String direc) {
		Session session = sesion.openSession();
		
		C1Profesores director = (C1Profesores) session.get(C1Profesores.class, dir);
		if (director == null) {
			System.out.println("El director " + dir + " no existe");
			session.close();
			return;
		}
		
		C1Centros centro = (C1Centros) session.get(C1Centros.class, cen);
		if (centro == null) {
			centro = new C1Centros();
			centro.setCodCentro(cen);
			centro.setNomCentro(nom);
			centro.setDireccion(direc);
			centro.setDirector(dir);
			
			session.persist(centro);
			System.out.println("El centro " + cen + " no existe. Se crea");
		} else {
			...
			tx.commit();
			System.out.println("El centro " + cen + " existe. Se actualiza");
		}
		
		C1Profesores prof = (C1Profesores) session.get(C1Profesores.class, 1000);
		if (prof == null) {
			System.out.println("El profesor " + prof.getCodProf() + " no existe");
			session.close();
			return;
		}
		
		if (centro.getC1Profesoreses().contains(prof)) {
			System.out.println("El centro ya tiene al profesor " + prof.getCodProf());
			session.close();
			return;
		}
		System.out.println("Se añade el profesor " + prof.getCodProf() + " al centro");
		
		Transaction tx = session.beginTransaction();
		
		centro.getC1Profesoreses().add(prof);
		session.merge(centro);
		tx.commit();
